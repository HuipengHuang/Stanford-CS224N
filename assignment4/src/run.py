...
if args.function == 'pretrain':
    ...
    ### YOUR CODE HERE ###
    pretrain_trainer_config=trainer.TrainerConfig(
    max_epochs=650,
    batch_size=128,
    learning_rate=args.pretrain_lr,
    lr_decay=True,
    ...
    num_workers=0,
    writer=writer)
    pretrain_corpus_path=args.pretrain_corpus_path
    save_path=args.writing_params_path
    pretrain_trainer=trainer.Trainer(model,pretrain_dataset,None,pretrain_trainer_config)
    ...
    save_model_params_path=args.writing_params_path
    ...
    if args.reading_params_path is None:
        ...
        model.load_state_dict(torch.load(args.reading_params_path,map_location='cuda'))
        model.to(device)
        trainer_config=trainer.TrainerConfig( max_epochs=10,
             ...
             num_workers=0,
             writer=writer)
    trainer=trainer.Trainer(model,train_dataset=finetune_dataset,test_dataset=None,config=trainer_config)
    trainer.train()
