...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_x=x[:length]
        masked_length=int(np.random.beta(1,3)*length)
        start=random.randint(0,length-masked_length)
        prefix=truncated_x[:start]
        masked_content=truncated_x[start:start+masked_length]
        suffix=x[start+masked_length:]
        x=prefix+[self.MASK_CHAR]+suffix+[self.MASK_CHAR]+masked_content
        x=x[:self.block_size]
        x=x+[self.PAD_CHAR]*(self.block_size+1-len(x))
        input=x[:-1]
        output=x[1:]
        input_tensor=torch.LongTensor([self.stoi[s] for s in input])
        output_tensor=torch.LongTensor([self.stoi[s] for s in output])
...
if __name__ == '__main__':
    ...
    if args.dataset_type == 'namedata':
        ...
        corruption_dataset = CharCorruptionDataset(
            open('C:/Users/27859\PycharmProjects\cs224n/assignment4/wiki.txt', encoding='utf-8').read(), 128)
        # Make the name dataset
        name_dataset = NameDataset(corruption_dataset,
            open('C:/Users/27859/PycharmProjects/cs224n/assignment4/birth_places_train.tsv', encoding='utf-8').read())
