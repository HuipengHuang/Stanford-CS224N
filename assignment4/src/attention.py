...
def precompute_rotary_emb(dim, max_positions):
    ...
    theta_ndarray=np.array([1/10000**(-2*(i-1)/dim) for i in range(int(dim/2))])
    cos_t_theta_list=[(np.cos(t*theta_ndarray)).tolist() for t in range(max_positions)]
    sin_t_theta_list = [(np.sin(t * theta_ndarray)).tolist() for t in range(max_positions)]

    rope_cache=torch.tensor([cos_t_theta_list, sin_t_theta_list])
...
def apply_rotary_emb(x, rope_cache):
    ...
    #shape x: batch_size,head, max_postions, dim
    rotated_x = None
    ### YOUR CODE HERE ###
    '''python src/run.py pretrain rope wiki.txt --writing_params_path rope.pretrain.params'''
    ...
    complex_rope_cache=torch.view_as_complex(rope_cache)
    x=x.reshape(x.shape[0],x.shape[1],x.shape[2],-1,2)
    complex_x=torch.view_as_complex(x)
    complex_rope_cache=complex_rope_cache[:,:complex_x.shape[-1]]
    rotated_x=complex_rope_cache*complex_x
    rotated_x=torch.view_as_real(rotated_x)
    rotated_x=rotated_x.reshape(rotated_x.shape[0],rotated_x.shape[1],rotated_x.shape[2],-1)
    pass
    ### END YOUR CODE ###
    return rotated_x
