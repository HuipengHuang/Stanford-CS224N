...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_partial_parse=partial_parses[:]
    while len(unfinished_partial_parse)>0:
        cycle=min(len(unfinished_partial_parse),batch_size)

        minibatch_partial_parses=unfinished_partial_parse[:cycle]
        predictions=model.predict(minibatch_partial_parses)
        for i in range(cycle):
            minibatch_partial_parses[i].parse_step(predictions[i])
        for i in range(cycle):
            if len(minibatch_partial_parses[i].buffer) == 0 and len(minibatch_partial_parses[i].stack)==1:
                unfinished_partial_parse.remove(minibatch_partial_parses[i])
    dependencies=dependencies+[partial_parses[i].dependencies for i in range(len(partial_parses))]
