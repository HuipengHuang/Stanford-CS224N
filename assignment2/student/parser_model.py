...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight=nn.Parameter(nn.init.xavier_uniform_(torch.zeros(size=(self.embed_size*self.n_features,self.hidden_size))))
        self.embed_to_hidden_bias=nn.Parameter(nn.init.uniform_(torch.zeros(size=(self.embed_size*self.n_features,self.hidden_size))))
        self.dropout=nn.Dropout(p=self.dropout_prob)
        self.hidden_to_logits_weight=nn.Parameter(nn.init.xavier_uniform_(torch.zeros(size=(self.hidden_size,self.n_classes))))
        self.hidden_to_logits_bias = nn.Parameter(
            nn.init.uniform_(nn.Parameter(torch.zeros(size=(self.hidden_size,self.n_classes)))))
    ...
    def embedding_lookup(self, w):
        ...
        for i in range(w.shape[0]):
            ...
            for j in range(w.shape[1]):
                index=int(w[i,j].item())
                v=self.embeddings[index]
                sub_tensor_list.append(v)
            x_list.append(torch.stack(sub_tensor_list,dim=0))
    ...
    def forward(self, w):
        ...
        w=w@self.embed_to_hidden_weight+w@self.embed_to_hidden_bias
        w=nn.ReLU()(w)
        w=self.dropout(w)
        logits=w@self.hidden_to_logits_weight+w@self.hidden_to_logits_bias
