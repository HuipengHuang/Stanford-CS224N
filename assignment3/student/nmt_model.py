...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        src_len,b,e=X.size()
        X=X.view(b,e,src_len)
        X=self.post_embed_cnn(X)

        X=X.view(src_len,b,e)
        X=pack_padded_sequence(X, torch.tensor(source_lengths))
        enc_hiddens,(last_hidden,last_cell)=self.encoder(X)

        enc_hiddens,length_tensor=pad_packed_sequence(enc_hiddens)
        enc_hiddens=enc_hiddens.view(b,src_len,-1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for t in torch.split(Y,1):
            ...
            dec_state, o_t, e_t=self.step(Y_bar_t,dec_state,enc_hiddens,enc_hidden_proj,enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t=torch.bmm(dec_hidden.unsqueeze(dim=1),enc_hiddens_proj.transpose(-1,-2))
